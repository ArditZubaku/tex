...
- **File explorer** — `<leader>e` (Space, then `e`) lists the directory the file being edited lives in, over the whole window the way `netrw` does rather than in a sidebar. Directories come first and carry a trailing `/`, `..` sits on top, and `hjkl` move the way they do in the buffer: `j`/`k` walk the listing, `Enter` or `l` descends into a directory or opens a file, `h` or `-` steps back out — landing on the directory just left rather than at the top — and `g`/`G` jump to the ends. `H` shows the dotfiles it hides by default, and `q` or `Esc` returns to the buffer. Opening a file swaps the buffer for it, resetting the cursor and the undo history; a buffer with unsaved changes refuses to be swapped (`E37`) the same way `:q` refuses to quit.
...
| `q` `Esc` | Explorer | return to the buffer |
