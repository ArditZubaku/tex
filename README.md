...
- **Line structure** — `o`/`O` open a line, `Enter` splits one at the cursor and `Backspace` at column 0 joins it back. Each shifts the line index in place rather than rebuilding it, and a split copies only the tail: the half before the cursor keeps the array it already had. A line any of the three starts is always indented one tab — a single space, the width `Tab` itself types — whatever the indent of the line it came from; `Backspace` right after takes that indent off before a second one joins the lines onto each other.
...
| `Enter` | Insert | split the line at the cursor and indent the new one by one tab (in Normal mode it moves down a line) |
...
| `o` | Normal | open a line below, indented by one tab, and enter Insert mode |
| `O` | Normal | open a line above, indented by one tab, and enter Insert mode |
