...
- **Ex commands** — `:` opens the same prompt the search does, for the commands a VIM user types without thinking: `:w` (and `:w other.txt`, which writes there and carries on editing that file, like `:saveas`), `:q`, `:wq`, `:x`, `:q!` to leave unsaved changes behind, `:e other.txt` to edit another file (`:e` on its own rereads the current one), `:bnext`/`:bprev`/`:bdelete`/`:ls` for the buffer list, `:nohlsearch` to drop the search highlight, and a bare line number — `:42`, `:$` — to jump there. `:q` on a modified buffer refuses and says so rather than losing the changes, and names the buffer when it is one of the others that is unsaved (`E162`); the raw `q` key still quits outright, without that check. Anything unrecognised is reported (`E492`) instead of guessed at.
