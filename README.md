...
internal/
  ...
  editor/               the loop: draw a frame, read a key, repeat
    ...
    find/               search, gd, gr, K, completion, code actions, the symbols, the diagnostics, the project grep, the popup
...
- **Auto-pairs** — typing `(`, `[`, `{` or `"` in Insert mode writes the closing half after the cursor, so the pair is never left half-written. Typing the closer the editor already wrote steps over it rather than doubling it, and `Backspace` between an empty pair takes both halves at once. A closer typed where there is nothing of its own kind to step over is inserted like any other character, which is what leaves unbalanced text still typeable. A quote is its own closer, and stepping over comes first: the second `"` finishes the string rather than opening another one.
- **Replace a character** — `r` waits for the next key and puts it in place of the rune under the cursor, staying in Normal mode: VIM's own `r`, down to the count, so `3rx` writes `xxx` over the three runes from the cursor and leaves it on the last of them. A count reaching past the end of the line replaces nothing at all rather than as much as it can, which is what VIM does with one, and the whole thing undoes in one step. Nothing means anything else while `r` is waiting — a digit is the replacement rather than a count, `Space` is a space rather than the leader — and a key with no rune to put in the file (`Esc`, `Enter`, an arrow) cancels and gives the keyboard back. It waits however long it takes, unlike a chord, which is why it is not held as one, and the status bar reads `REPLACE:` for as long as it does so the wait is never a keystroke thrown away.
- **Line structure** — `o`/`O` open a line, `Enter` splits one at the cursor and `Backspace` at column 0 joins it back. Each shifts the line index in place rather than rebuilding it, and a split copies only the tail: the half before the cursor keeps the array it already had. A line any of the three starts carries the indent of the line it came from, copied verbatim so a file written with real tabs stays that way, and one level deeper when that line ends on a bracket it left open — a tab where the copied indent has one, otherwise the single space `Tab` itself types. `O` is the exception to the deepening: the bracket the line below it opens is not a block the new line sits inside. `Enter` between a pair the editor closed itself — the `{}` typing `{` leaves behind — opens the block out over three lines, the cursor on an indented empty one and the closer back at the indent of the line that opened it.
- **Comment toggling** — `gcc` comments or uncomments the current line, or with a count the lines below it too (`3gcc`); `gc` does the same over a Visual selection, covering every line it touches whether the selection runs by rune or by line. The leader is whichever language's own — `//` for Go and the C family, `#` for Python, the shells, YAML and the rest — read off the same per-language table the syntax highlighting below colours comments by, so a file of no recognised kind leaves `gcc` a no-op rather than guessing one. A block reads as already commented only once every non-blank line in it carries the leader, in which case they all lose it together; otherwise every line still missing it gains one and a line that already had it is left alone, so toggling a block commented by hand finishes the job instead of doubling up. Blank lines are left alone either way, indentation in front of the leader survives it, and the whole block undoes in one step.
...
- **Counts** — a command can be prefixed with a repeat count, as in `3j`, `3x`, `2dd` or `yy3p`. Commands where the count says *how much text* rather than *how many times* (`x`, `r`, `dd`, `yy`, `p`, `P`) act on that much text in a single step, so one `u` takes the whole thing back. A count in front of `%` means something else again — a percentage of the file rather than a repeat — which is VIM's own reading of it.
- **Matching brackets** — `%` jumps to the match of the bracket under the cursor, or of the first one after it on the line, across as many lines as it takes: `(`/`)`, `[`/`]` and `{`/`}`, which are VIM's own `matchpairs`. Nesting is counted and only the same kind counts, so the `(` of `([)]` finds the `)` rather than the `]`. It reads the text and nothing more — a bracket inside a string or a comment is a bracket, the way VIM's own `%` sees one — and a line with no bracket on it, or a bracket with no match, leaves the cursor where it is. It counts as a jump, so `Ctrl-O` comes back from it. A quote is not a pair here: both halves of one are the same rune, so there is no telling which half the cursor is on. With a count in front it is VIM's other `%` entirely — `50%` goes to the line halfway through the file, on its first non-blank — and a percentage past a hundred is no part of the file, so it is refused rather than rounded down to the end.
...
- **Relative line numbers** — a gutter on the left showing each line's distance from the cursor, with the cursor's own line carrying its absolute number instead, like VIM's `number`/`relativenumber` pair. The column is set in one place from the window's left edge, so the cursor's own number — ranged left where every other is ranged right — does not sit against the frame. It widens with the buffer's line count (five columns until the count reaches four digits) and the text and horizontal scrolling start after it.
...
- **Ex commands** — `:` opens the same prompt the search does, for the commands a VIM user types without thinking: `:w` (and `:w other.txt`, which writes there and carries on editing that file, like `:saveas`), `:q`, `:wq`, `:x`, `:q!` to leave unsaved changes behind, `:e other.txt` to edit another file (`:e` on its own rereads the current one), `:bnext`/`:bprev`/`:bdelete`/`:ls` for the buffer list, `:split`/`:vsplit`/`:close`/`:only` for windows, `:nohlsearch` to drop the search highlight, `:rename` for the identifier under the cursor, `:wa` to write every buffer that has unsaved changes, `:qa` to quit however many windows are open, and a bare line number — `:42`, `:$` — to jump there, which counts as a jump so `Ctrl-O` comes back from it. `:q` closes the window it is typed in and quits once that was the last one. Anything unrecognised is reported (`E492`) instead of guessed at.
- **Themes** — `:theme=2` switches the whole palette, `:theme=1` goes back, and `:theme` on its own names what is in use. Four so far: **1** the editor's own colours, the only one that leaves the terminal's background alone; **2** [gruvbox](https://github.com/morhetz/gruvbox) — red keywords, yellow types, green strings and bold green functions, purple literals, aqua built-ins, grey comments over its `bg0`; **3** GitHub's dark — red keywords, green type names, purple functions, blue literals, light-blue strings over `#0d1117`; **4** GitHub's light default (the one the editor starts in) — the same palette over `#ffffff`. All are the nearest 256-colour index to each hex value the palette itself publishes. Every colour the editor draws in, syntax and interface alike, comes out of the theme in use, so a fifth one is a value in `internal/theme` and nothing else.
...
- **Code actions** — `<leader>ca` asks a language server what it can do about the cursor's position and lists the answers in the same popup as everything else, even when there is only one: gopls's fix for a missing method falls out of this the same as any other, needing nothing special here since it is a quickfix like the rest of what the menu can hold. `Enter` applies whichever is chosen and undoes in one step; the status line reports which one (`applied: ...`) or why not.

  It applies an edit already in the answer, or gets one first when the server held it back as `data` instead of sending it eagerly — gopls answers most of its own fixes that way, the missing-method one included, and `codeAction/resolve` is what turns that into the edit there was never any other way to get. A code action naming a `Command` instead — run through `workspace/executeCommand`, which nothing here speaks — is listed but refused if it is the one chosen (`code action runs a command, not yet supported: ...`). An edit that reaches into a file other than the one open is refused whole rather than applied in part (`code action reaches beyond this file, not yet supported: ...`), and the same goes for one that creates, renames, or deletes a file rather than just editing it (`code action creates, renames, or deletes a file, not yet supported: ...`), since some of a fix landing is worse than none of it. There is nothing behind it in the text, the same as `K`: with no server running `<leader>ca` says so, and a server with nothing to offer at the cursor says that instead (`no code actions here`).
...
| `%` | Normal | jump to the matching bracket (`50%` goes half way through the file instead) |
| `x` | Normal | delete the character under the cursor |
| `r` | Normal | replace the character under the cursor with the next key typed (`3rx` replaces three) |
...
| `gc` | Visual | toggle the line comment over every line the selection touches |
...
| `<leader>ca` | Normal | list what a language server can do about the cursor, and apply the one chosen |
...
| `r` | Explorer | rename the entry, opening on the name it already has |
...
| `:theme=2` | Normal | switch palette (`1` editor's own, `2` gruvbox, `3` github-dark, `4` github-light, default; bare `:theme` names it) |
...
| `(` `[` `{` `"` | Insert | insert it and its closing pair, leaving the cursor between them |
| `)` `]` `}` `"` | Insert | step over the closer when it is the one already under the cursor, insert it otherwise |
| `Backspace` | Insert | delete the character before the cursor, taking the closing half too when it sits in an empty pair, and joining onto the line above at column 0 |
| `Tab` | Insert | insert one space (with a completion menu up it settles on a candidate instead) |
| `Enter` | Insert | split the line at the cursor and indent the new one to match the half above it, a level deeper when that half opens a bracket, with the closer of a pair dropped to a line of its own (in Normal mode it moves down a line) |
...
| `o` | Normal | open a line below, indented to match the line above it and a level deeper when that line opens a bracket, and enter Insert mode |
| `O` | Normal | open a line above, indented to match the line it pushes down, and enter Insert mode |
